...
@WebServlet(name = "InscricaoCarregar", urlPatterns = {"/InscricaoCarregar"})
public class InscricaoCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Inscricao oIns = (Inscricao) dao.carregar(idInscricao);
            
            Gson ogson = new GsonBuilder().setDateFormat("yyyy-MM-dd").create();
            String jSon = ogson.toJson(oIns);
            response.getWriter().write(jSon);
            
        } catch (Exception ex){
            System.out.println("Erro carregar inscricao"+ex.getMessage());
            ex.printStackTrace();
        }
    }
