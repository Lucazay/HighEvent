...
@WebServlet(name = "DoacaoCarregar", urlPatterns = {"/DoacaoCarregar"})
public class DoacaoCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Doacao oDoa = (Doacao) dao.carregar(idDoacao);
            
            Gson ogson = new GsonBuilder().setDateFormat("yyyy-MM-dd").create();
            String jSon = ogson.toJson(oDoa);
            response.getWriter().write(jSon);
            
        } catch (Exception ex){
            System.out.println("Erro carregar doacao"+ex.getMessage());
            ex.printStackTrace();
        }
    }
