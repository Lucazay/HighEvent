 ...
 * @author Acer
 */
@WebServlet(name = "ParcelamentoCarregar", urlPatterns = {"/ParcelamentoCarregar"})
public class ParcelamentoCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            Parcelamento oParc = (Parcelamento) dao.carregar(idParcelamento);
            
            Gson ogson = new GsonBuilder().setDateFormat("yyyy-MM-dd").create();
            String jSon = ogson.toJson(oParc);
            response.getWriter().write(jSon);
            
        }catch(Exception ex){
            System.out.println("Erro ao buscar Participante- "+ex.getMessage());
            ex.printStackTrace();
        }
    }
