 ...
 * @author Lucas_Kenji
 */
@WebServlet(name = "EstornoCarregar", urlPatterns = {"/EstornoCarregar"})
public class EstornoCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Estorno oEst = (Estorno) dao.carregar(idEstorno);
            
            Gson ogson = new GsonBuilder().setDateFormat("yyyy-MM-dd").create();
            String jSon = ogson.toJson(oEst);
            response.getWriter().write(jSon);
            
        } catch (Exception ex){
            System.out.println("Erro carregar estorno"+ex.getMessage());
            ex.printStackTrace();
        }
    }
