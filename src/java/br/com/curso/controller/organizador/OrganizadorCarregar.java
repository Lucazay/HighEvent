...
@WebServlet(name = "OrganizadorCarregar", urlPatterns = {"/OrganizadorCarregar"})
public class OrganizadorCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            Organizador oOrg = (Organizador) dao.carregar(idOrganizador);
            
            Gson ogson = new GsonBuilder().setDateFormat("yyyy-MM-dd").create();
            String jSon = ogson.toJson(oOrg);
            response.getWriter().write(jSon);
            
        }catch(Exception ex){
            System.out.println("Erro ao buscar Organizador- "+ex.getMessage());
            ex.printStackTrace();
        }
    }
